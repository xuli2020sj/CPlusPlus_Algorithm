...
// 
// Related Topics 设计 数组 哈希表 链表 哈希函数 👍 302 👎 0
...
public:
    ...
    void put(int key, int value) {
        ...
        for (auto iter = data[hash_val].begin(); iter != data[hash_val].end(); ++iter) {
            if (iter->first == key) {
                iter->second = value;
                return;
            }
        }
        data[hash_val].push_back({key, value});
    }
    
    int get(int key) {
        size_t hash_val = hash(key);
        for (auto iter : data[hash_val]) {
